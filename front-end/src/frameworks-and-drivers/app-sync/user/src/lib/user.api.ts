...
export class UserApi implements UserRepository {
  ...
  async login(phoneNumber: string, password: string): Promise<User & { accessToken: string }> {
    return axios.post<User & { accessToken: string }>(`/users/login`, {phoneNumber, password})
      .then((response) => response.data)
      .catch((error) => {
          throw error;
        }
  ...
  async verifyPhoneNumber(phoneNumber: string): Promise<boolean> {
    return axios.get<{ isUsed: boolean }>(`/users/verify/${phoneNumber}`)
  ...
  async register(user: User): Promise<{ userId: string }> {
    return axios.post<{ userId: string }>(`/users`, user)
      .then((response) => response.data)
      .catch((error) => {
          throw error;
        }
