...
export const NotificationCard = (props: NotificationCardProps) => {
  ...
  if (timeDiff < oneMin) {newTime = Math.round(timeDiff / 1000); timeString = newTime + ' secs ago'}
  else if (timeDiff < oneHour) {newTime = Math.round(timeDiff / oneMin); timeString = newTime + ' mins ago'}
  else if (timeDiff < oneDay) {newTime = Math.round(timeDiff / oneHour); timeString = newTime + ' hours ago'}
  else {newTime = Math.round(timeDiff / oneDay); timeString = newTime + ' days ago'}
      ...
      <View style={styles.content}>
        ...
        <View style={{paddingLeft: ScreenSize.width * 0.04}}>
          ...
          <View style={{flexDirection: 'row'}}>
            <MaterialCommunityIcons name="clock-outline" size={ScreenSize.width * 0.05} color={isRead ? Color.grey_100 : Color.primary_100}></MaterialCommunityIcons>
