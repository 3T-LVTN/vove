...
export function TrackingList(props: TrackingListProps) {
  ...
  return (
    <View style={styles.container}>
      ...
      {selected === 0
        ? trackingSummaryList.map((item) => <TrackingSummaryCard key={item.id} status={item.status}
                                                                 navigation={navigation} address={item.address}
                                                                 notificationAllowed={item.notificationAllowed}
                                                                 placeName={item.placeName} editable={true}/>)
        : selected === 1
          ? trackingSummaryList
            .filter((item) => item.status === TrackingPlaceStatusType.GOOD)
            .map((item) => <TrackingSummaryCard key={item.id} status={item.status} navigation={navigation}
                                                address={item.address} notificationAllowed={item.notificationAllowed}
                                                placeName={item.placeName} editable={true}/>)
          : selected === 2
            ? trackingSummaryList
              .filter((item) => item.status === TrackingPlaceStatusType.LOW_RISK)
              .map((item) => <TrackingSummaryCard key={item.id} status={item.status} navigation={navigation}
                                                  address={item.address} notificationAllowed={item.notificationAllowed}
                                                  placeName={item.placeName} editable={true}/>)
            : selected === 3
              ? trackingSummaryList
                .filter((item) => item.status === TrackingPlaceStatusType.HIGH_RISK)
                .map((item) => <TrackingSummaryCard key={item.id} status={item.status} navigation={navigation}
                                                    address={item.address}
                                                    notificationAllowed={item.notificationAllowed}
                                                    placeName={item.placeName} editable={true}/>)
              : trackingSummaryList
                .filter((item) => item.status === TrackingPlaceStatusType.EPIDEMIC)
                .map((item) => <TrackingSummaryCard key={item.id} status={item.status} navigation={navigation}
                                                    address={item.address}
                                                    notificationAllowed={item.notificationAllowed}
                                                    placeName={item.placeName} editable={true}/>)
