...
const notificationList = [
  ...
  },
  {
    leftIcon: '',
    content:
      "Admin has replied your inquiry 'Add support for multiple languages'",
  ...
  },
  {
    leftIcon: '',
    content:
      "Admin has opened your inquiry 'Add support for multiple languages'",
      ...
      <ScrollView style={styles.container}>
        ...
        <View style={{ flexDirection: 'row' }}>
          {filterTab.map((e) => (
            <ButtonSmall
              type={
                status == e.status ? ButtonType.DEFAULT : ButtonType.OUTLINE
              }
              content={e.status}
        ...
        <View>
          ...
          {notificationList
            .filter((newNof) => Number(new Date()) - newNof.time < oneDay)
            .filter((readNof) =>
              status == 'Unread' ? readNof.isRead == false : readNof
            )
            .map((nof) => (
              <View>
                <NotificationCard
                  leftSymbol={nof.leftIcon}
                  content={nof.content}
                  time={nof.time}
                  isRead={nof.isRead}
                />
          ...
          {notificationList
            .filter((oldNof) => Number(new Date()) - oldNof.time >= oneDay)
            .filter((readNof) =>
              status == 'Unread' ? readNof.isRead == false : readNof
            )
            .map((nof) => (
